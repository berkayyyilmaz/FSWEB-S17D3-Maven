...
public class KangarooController {
    ...
    public Kangaroo update(@PathVariable Integer id,@RequestBody Kangaroo kangaroo ){
        ZooKangarooValidation.isIdValid(id);
        ZooKangarooValidation.checkKangarooWeight(kangaroo.getWeight());
        kangaroo.setId(id); //path variable'daki id, ilk id. ama request body'de başka bir id gönderiyor olabilir. Ben eski kaydın id'si kalsın istiyorum bu nedenle body'deki id'yi setliyoruz istekteki id ile.
        if (kangaroos.containsKey(id)){
            kangaroos.put(id,kangaroo);
            return kangaroos.get(id);
        }
        else {
            return save(kangaroo); //varsa güncellesin yoksa(else); kaydetsin yeni.
        }
    }
